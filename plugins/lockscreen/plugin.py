        ...
        main_hooks=[
            MainHook(
                ...
                code=(
                    ...
                    "    // BootResume::Silent covers internal silentRestart() cycles used to\n"
                    "    // reset WiFi/webserver state (e.g. leaving File Transfer or the OPDS\n"
                    "    // browser) - the user never actually slept, so don't lock for those.\n"
                    "    // QuickResume (waking from sleep) and Splash (real power-on) still lock.\n"
                    "    const bool shouldLock = hasPinSet && resume != BootResume::Silent &&\n"
                    "                            LockscreenPlugin::shouldLock(lsMode);\n"
                    "    if (shouldLock) {\n"
