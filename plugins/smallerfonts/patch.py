    ...
    for family, size, folder, ext, styles in new_fonts:
        for style in styles:
            ...
            if not os.path.exists(src_path):
                src_path = os.path.join(source_dir, folder, f"{folder.replace('NotoSans','NotoSans')}-{style}.{ext}")
...
def patch_settings_cpp(repo_dir):
    ...
    content = content.replace(
        ...
        '      if (selectedCategoryIndex == 4) return "Smaller Fonts";\n'
